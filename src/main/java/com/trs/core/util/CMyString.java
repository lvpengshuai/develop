...
public class CMyString {
     ...
     * </p>
     *
     * @param _sContent     要显示的内容
     * @param _bChangeBlank 是否转换空格符；可选参数，默认值为true.
     * @return 转化后的Html文本
     */
     ...
     * </p>
     *
     * @param _sContent     文本内容；
     * @param p_sQuoteColor 引用语的显示颜色。
     * @param _bChangeBlank 是否转换空格符，可省略，默认值为true
     * @return 转化后的Html文本
     */
     ...
     */
    public static String truncateStr(String _string, int _maxLength,
                                     String _sExt, boolean _bFullLen) {
        if (_string == null) {
            return null;
        }
        ...
        int iRemain = _bFullLen ? _maxLength : (_maxLength - nExtLen);
    ...
    private static void loadFirstLetter(String _sFileName) throws Exception {
        ...
        try {
            ...
            throw new Exception("要读取的拼音配置文件没有找到(CMyString.getFirstLetter)");
        } catch (IOException ex) {
            throw new Exception("读拼音配置文件件时错误(CMyString.getFirstLetter)");
        } finally {
            if (buffReader != null)
                buffReader.close();
            if (fis != null)
     ...
     * 替换内容中的变量
     *
     * @param _sContent  正文内容
     * @param _variables 变量表
     * @return
     * @throws WCMException
     */
     ...
     * @param _sContent  正文内容
     * @param _variables 变量表
     * @param _aSep      变量前缀和后缀分隔符
     * @return
     * @throws WCMException
     */
    public static String parsePageVariables(String _sContent, Map _variables,
                                            char[] _aSep) throws Exception {
        return parsePageVariables(_sContent, _variables, _aSep, 20);
    }
     ...
     * @param _sContent            正文内容
     * @param _variables           变量表
     * @param _nPgvarnameMaxLength 变量最大匹配长度
     * @return
     * @throws WCMException
     */
    public static String parsePageVariables(String _sContent, Map _variables,
                                            int _nPgvarnameMaxLength) throws Exception {
        return parsePageVariables(_sContent, _variables,
     ...
     */
    public static String parsePageVariables(String _sContent, Map _variables,
                                            char[] _aSep, int _nPgvarnameMaxLength) throws Exception {
        if (_sContent == null) {
            return null;
        }
        ...
        try {
            ...
            throw new Exception("解析内容中的变量失败!");
        }
    }
    ...
    public static String format(String _sFormat, String[] _args) {
        ...
        for (int i = 0; i < _args.length; i++) {
            String param = CMyString.showNull(_args[i]).replaceAll("(\\$)",
                    "\\\\$1");
    ...
    public static String innerText(String _strValue) {
        String sValue = _strValue.replaceAll("(?is)<style[^>]*>.*</style>", "");
     ...
     *
     * @param string      需要判断定字符串
     * @param targetArray 指定的字符串数组
     * @param ignoreCase  是否忽略大小写比较,当该参数值为<code>true</code>时,忽略大小写
     ...
     * @param arArray
     * @param _sFind
     * @return 指定字符串在数组中的位置，-1代表未找到
     */
    public static int find(String[] arArray, String _sFind) {
        if (arArray == null || arArray.length == 0)
            return -1;

        int nIndexOf = -1;
        for (int i = 0; i < arArray.length; i++) {
            if (arArray[i].equalsIgnoreCase(_sFind)) {
                nIndexOf = i;
                break;
            }
        }
