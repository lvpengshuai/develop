 ...
 */
@Controller
public class ErrorLogController extends AbstractAdminController {

    @Resource
    private LogService logService;

    /*跳转日志页面*/
    @Permission(url = "/error-record")
    @RequestMapping(value = "/error-record", method = RequestMethod.GET)
    public ModelAndView toLog() {
        ModelAndView modelAndView = new ModelAndView("admin/log/errorlog");
        modelAndView.addObject("title", "异常日志");

        return modelAndView;
    }

    @ResponseBody
    @RequestMapping(value = "/error-records", method = RequestMethod.GET)
    ...
    @ResponseBody
    @RequestMapping(value = "/error-record/{ids}", method = RequestMethod.DELETE)
