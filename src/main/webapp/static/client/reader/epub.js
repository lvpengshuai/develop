...
用childNodes代替querySelectorAll，后者会把孙子节点也查出来，前者是亲子节点
*/
(function() {
  var registry = {}, seen = {};
...
EPUBJS.Book = function(options){
	...
	*/
	...
	this._displayQ = [];
	this._displayQCompare = [];
	this.calcTotalPaging = false;
...
EPUBJS.Book.prototype.open = function(bookPath, forceReload){
	//alert(bookPath);
	//console.log(1, "open epub:", bookPath); // :: 1 a/
	var book = this,
			epubpackage,
	...
	//console.log("line 1897", this.bookUrl); // :: 3  /a/

	if(this.settings.contained || this.isContained(bookPath)){ // 解压epub或zip文件
		//alert(1);
	...
	//console.log(9, this.settings.restore, forceReload);
	if(this.settings.restore && !forceReload){
		...
		epubpackage.then(function(packageXml) {
			...
			//console.log(10, identifier, restored); // 10 (空字符串) false

			// 不知道为什么该值为true时，会导致拿不到chapters
			//if(!restored) {
...
EPUBJS.Book.prototype.loadPackage = function(_containerPath){
	var book = this,
        ...
        // META-INF
			containerPath = _containerPath || "METAINF/container.xml",
			containerXml,
			packageXml;
	
	if(!this.settings.packageUrl) { //-- provide the packageUrl to skip this step  opf
		//console.log(book.bookUrl + containerPath); // :: 4
		packageXml = book.loadXml(book.bookUrl + containerPath).
			then(function(containerXml){
				//console.log(containerXml); // :: 5
				return parse.container(containerXml); // Container has path to content
			}).
			then(function(paths){
				//console.log("line 1965", paths); // :: 6
				...
				//console.log(book.settings.packageUrl); // :: 7
	...
	//console.log("line 1976", packageXml); // Promise
	return packageXml;
};
...
EPUBJS.Book.prototype.unpack = function(packageXml){
	...
	for(var i = 0; i < book.spine.length; i++){
		this.chapters.push(new EPUBJS.Chapter(this.spine[i]));
		//EPUBJS.core.request(this.chapters[i].absolute);
	}
...
//				
//				var oldLength = 0;
//				while(oldLength != book.chapters.length){
//					oldLength = book.chapters.length;
//					book.chapters.forEach(function(chapterItem,chapterI){
//						var del = true;
//						chapterItem.
//						book.toc.forEach(function(tocItem){  
//							if(tocItem.href == chapterItem.href){
//						    	del = false;
//						    	return;
//						    }else{
//							    tocItem.subitems.forEach(function(subitem){
//							    	 if(subitem.href == chapterItem.href){
//									    	del = false;
//									    	return;
//									  };
//							    });
...
//						});
//						if(del){
//							book.chapters.baoremove(chapterI);
//							return;
//						}
//					});
//				}
//				
				book.chapters.forEach(function(chapterItem,chapterI){
					chapterItem.spinePos = chapterI;
					book.spineIndexByURL[chapterItem.href]=chapterI;
				});
		...
		book.loadXml(book.settings.tocUrl).
			then(function(tocXml){
				...
				book.toc = book.contents.toc = toc;
				//console.log(13, toc); //　和１２处的book.contents.toc一样，为什么要再次处理？
...
//						var del = true;
//						book.toc.forEach(function(tocItem){  
//							if(tocItem.href.indexOf(chapterItem.href) >-1 ){
//						    	del = false;
//						    	return;
//						    }else{
//							    tocItem.subitems.forEach(function(subitem){
//							    	 if(subitem.href.indexOf(chapterItem.href) >-1){
//									    	del = false;
//									    	return;
//									  };
//							    });
...
EPUBJS.Book.prototype.startDisplay = function(){
	//console.log("line 2222", this.spine, this.settings.previousLocationCfi, this.spinePos);
	var display;
	
	// 判断是否需要计算总页码
	if(this.settings.calcBookPages) {
...
EPUBJS.Book.prototype.restore = function(identifier){
	...
	console.log("2275", reject, !fromStore, !this.contents, !this.settings.contentsPath);
...
EPUBJS.Book.prototype.displayChapter = function(chap, end){
	//console.log("line 2291", chap, end);
	this.currentChapter = chap;
	var book = this,
		render,
		cfi,
		pos;
	//console.log("line 2281", chap, end, this.isRendered, this._rendering); // /6/26[idchapter011_13]!/4/406
	if(!this.isRendered) return this._enqueue("displayChapter", arguments);
	
	if(this._rendering) {
		//console.log(this._rendering, arguments, typeof arguments);
		var contained = false;
		book._displayQCompare.forEach(function(item){
			if (item[0] == chap) {
				contained = true;
			}
		});
		//console.log("line 2305", contained, chap, this._displayQCompare);
		if (!contained) {
			this._displayQ.push(arguments);
			this.calcTotalPaging && this._displayQCompare.push(arguments)
		}
		//console.log(this._displayQ)
		return;
	}

	//console.log("line 2314", chap);
	// 如果是数字，直接导航，如果是字符串，分割之后导航
	if(_.isNumber(chap)){
		pos = chap;
	}else{
	...
	//console.log("line 2323", cfi);
	// 翻页到最后一页时，会导般到第一页去，是这里起的作用
	if(pos < 0 || pos >= this.spine.length){
		console.warn("Not A Valid Location");
		//pos = 0;
		pos = (pos >= this.spine.length ? pos - this.spine.length : 0);
		// 最后一页
		book.trigger("book:lastestPage");
		end = false;
		cfi = false;
	}
	...
	this.spinePos = pos;
	//console.log("line 2335",pos, book.chapters);
	//-- Create a new chapter	
	this.chapter = book.chapters[pos] || new EPUBJS.Chapter(this.spine[pos]);
	//console.log(15, this.chapter);
	...
	if(cfi) {
		render.then(function(chapter){
			//console.log("line 2346", chapter, chap); //  Object  /6/8[idchapter003_5]!/4/24
	...
	if(!this.settings.fromStorage &&　!this.settings.calcBookPages && this.settings.preloadNextChapter &&
	...
	render.then(function(){
		var inwait;
		//console.log("line 2343 render.then");
		book._rendering = false;
		if(book._displayQ.length) {
			//inwait = book._displayQ.unshift();
			inwait = book._displayQ.shift();
			//console.log(typeof inwait, inwait, book._displayQ);
			//setTimeout(function(){
				book.displayChapter.apply(book, inwait);
			//});
		} else {
			book_ready_complete = false;
			//console.log("line 2353");
			var totalPages = 0;
			//console.log("line 2381", book.chapters);
			book.chapters.forEach(function(chap) {
				totalPages += chap.pages;
			});
			if (isNaN(totalPages)) {
				//console.log("总页数非法！！！！！！",　totalPages);
				return;
			}
			console.log("总页码为", totalPages);
			book.totalPages = totalPages;
			book.calcTotalPaging = false;
			book._displayQCompare = [];
			
			book_ready_complete = true;
			
			book.trigger("book:readyComplete");
		}
...
EPUBJS.Book.prototype.calcBookPages = function() {
	this.calcTotalPaging = true;
	// IE中会出现这样的值　/6/2[Section0000.html]! 1 /6/4[Section0017.xhtml]!
	//console.log("line 2405  计算总页数", this.settings.previousLocationCfi, this.currentChapter, this.getCurrentLocationCfi());
	var i, cfi, pos = 0;
	var chap = this.getCurrentLocationCfi() || this.settings.previousLocationCfi;
	//console.log("line 2397", chap);
	if (_.isNumber(chap)) {
		pos = this.currentChapter;
	} else {
		cfi = new EPUBJS.EpubCFI(chap);
		pos = cfi.spinePos;
	}
	//console.log("line 2404", cfi, pos, this.chapters.length);
	for(i = pos + 1; i < this.chapters.length; i++) {
		this.displayChapter(i);
	}
	for(i = 0; i < pos; i++) {
		this.displayChapter(i);
	}
	this.displayChapter(chap || 0);
};

EPUBJS.Book.prototype.gotoPage = function(pageIndex){
	this.render.gotoPage(pageIndex);
};
...
EPUBJS.Book.prototype.nextPage = function(){
	...
	//console.log("line 2362 next=", next);
...
EPUBJS.Book.prototype.gotoCfi = function(cfi){
	//console.log(cfi); // /6/8[idchapter003_5]!/4/24
...
EPUBJS.Book.prototype.goto = function(url){
	...
	console.log(this.spineIndexByURL);
	spinePos = this.spineIndexByURL[chapter];
	console.log(spinePos); // alert 9;
	...
	console.log(this.chapter.spinePos, spinePos);
...
EPUBJS.Book.prototype.preloadNextChapter = function() {
	...
	next = this.chapters[chap];//new EPUBJS.Chapter(this.spine[chap]);
	//console.log("nextChapter", next);
	if(next) {
		EPUBJS.core.request(next.absolute, 'html');
	}
};
...
EPUBJS.Book.prototype._enqueue = function(command, args) {
	console.log("line 2559", command, args);
	this._q.push({
		'command': command,
		'args': args
	});
...
EPUBJS.Chapter = function(spineObject){//console.log("line 2713", spineObject);
...
EPUBJS.core.request = function(url, type, withCredentials) {
	...
	var xhr = createCORSRequest();//new XMLHttpRequest();
...
EPUBJS.EpubCFI.prototype.generateFragment = function(element, chapter) {
	var path = this.pathTo(element),
		parts = [];
	//console.log("line 3088 generateFragment", path); // /6/4[Section0017.xhtml]!
...
EPUBJS.EpubCFI.prototype.parse = function(cfiStr) {
	...
	//console.log(cfiStr); // /6/8[idchapter003_5]!/4/24
	cfi.chapter = this.getChapter(cfiStr);
	//console.log(cfi.chapter); // /6/8[idchapter003_5]
	
	chapSegment = parseInt(cfi.chapter.split("/")[2]) || false;
	//console.log(chapSegment); // 8
	
	cfi.fragment = this.getFragment(cfiStr);
	//console.log("line 3168", cfi.fragment); // /4/24

	if(!chapSegment || !cfi.fragment) return {spinePos: -1};
	
	cfi.spinePos = (parseInt(chapSegment) / 2 - 1 ) || 0;
	//console.log(cfi.spinePos); // 3

	chapId = cfi.chapter.match(/\[(.*)\]/);

	cfi.spineId = chapId ? chapId[1] : false;
	//console.log(cfi.spineId); // idchapter003_5

	path = cfi.fragment.split('/');
	//console.log(path); // ["", "4", "18"]
	...
	path.forEach(function(part){
		...
		//console.log(index, id); // 1 undefined,  8 undefined
		cfi.sections.push({
			'index' : index,
			'id' : id || false
		});
...
EPUBJS.EpubCFI.prototype.getElement = function(cfi, _doc) {
	...
	// sections.shift(); //-- html
	//console.log(sections); // [Object { index=1, id=false}, Object { index=12, id=false}]
	...
	while(sections && sections.length > 0) {
		...
		if(part.id){
			...
			//console.log(children); // 文档内容
			//console.log(part.index); // 1, 12
		...
		children = Array.prototype.slice.call(element.children);
		//console.log(children); // [p,p,p,p,p....],[]
	}
...
EPUBJS.Parser.prototype.toc = function(tocXml, spineIndexByURL, bookSpine){
	...
	function getTOC(parent){
		var list = [],
				items = [],
				nodes = parent.childNodes,//parent.querySelectorAll("navPoint"),
		...
		items.forEach(function(item){
			if (item.nodeName != "navPoint") return;
			var id = item.getAttribute('id') || false,
...
EPUBJS.Renderer.prototype.reformat = function(){
	...
	setTimeout(function(){
		//console.log("reformat calcpages")
...
EPUBJS.Renderer.prototype.setIframeSrc = function(url){
	...
	this.visible(false);
	//alert("EPUBJS.Renderer.prototype.setIframeSrc="+url);
	//alert(EPUBJS.readDataPath);
//	if (EPUBJS.isReadData == true) {
//		$.ajax({
//			type : "POST",
//			url : EPUBJS.readDataPath + "?chapterUrl=" + url,
//			cache : false,			
//			dataType : "json",
//			success : function(data) {
//				if(data=="1001010"){
//					window.location.href="http://www.dzzgsw.com/login";  
//				} else {
//					url = url + "?serializedHighlights=" + data;
//				}
//			},
...
//		});
//	}
	var iiframe_v = this.iframe;
	if (EPUBJS.isReadData == true && book_ready_complete) {
		$.ajax({
			type : "POST",
			//url : "http://192.168.31.239:8080/publishweb/book/webreadata/get",
			url : EPUBJS.readDataPath,
			data:{
			...
			success : function(data) {
				rangy_chapterUrl = url;
				rangy_hilighter_string = data;
				iiframe_v.src = url+"?serializedHighlights="+data;
			},
			error : function(xhr, status, err) {
   ...
   // 	var framedoc = document.getElementById('ifName').contentWindow.document;  
    	//$(framedoc).find("p ").css({"line-height":"2.5em"});
		...
		//$("#viewer > iframe").attr("name","ifName");
    	//$(window.frames["ifName"].document).find("p ").css({"line-height":"2.5em"});
...
EPUBJS.Renderer.prototype.formatSpread = function(){
	...
	//console.log("line 4020", divisor, this.spreadWidth);
	// if(this.bodyEl) this.bodyEl.style.margin = 0;
	// this.bodyEl.style.fontSize = localStorage.getItem("fontSize") || "medium";
	//console.log("line 3982   阅读宽度:", this.elWidth, this.gap, this.spreadWidth, this.colWidth, divisor);
	...
	//this.docEl.style.position = "relative";
	//this.docEl.style.zoom = 1;
	//this.docEl.style.MozBoxSizing = "border-box";

	//-- Adjust height
	this.docEl.style.height = this.iframe.clientHeight	+ "px";
	//console.log(EPUBJS.Renderer.columnWidth);

	//-- Add columns
	this.docEl.style[EPUBJS.Renderer.columnAxis] = "horizontal"; // firefox中无效？
...
EPUBJS.Renderer.prototype.calcPages = function() {
	...
	//console.log("line 4065", this.totalWidth, this.spreadWidth);
	this.displayedPages = Math.ceil(this.totalWidth / this.spreadWidth);

	this.currentChapter.pages = this.displayedPages;
	//console.log("epub.js line 4053", this.displayedPages, this.currentChapter.absolute, this.book.chapters);
//	this.book.chapters.forEach(function(item) {
//		console.log(item.absolute, item.pages);
//	});
	}catch(e){}
};

EPUBJS.Renderer.prototype.nextPage = function(){
	//console.log(this.chapterPos, this.displayedPages); // alert 6 // 当前章节总页数及当前页
...
EPUBJS.Renderer.prototype.gotoPage = function(pageIndex) {
	var $this = this;
	var chap, chapterPos, pages = 0;
	for(var i = 0; i < this.book.chapters.length; i++) {
		var ch = this.book.chapters[i];
		if (pages + ch.pages > pageIndex) {
			chap = i;
			//console.log("找到了", i);
			break;
		} else {
	...
	if (pages - 1 != pageIndex) {
		chapterPos = pageIndex - pages;
	}
	//console.log("line 4176", chap, chapterPos, pages, pageIndex);
	if (chap) {
		var render = this.book.displayChapter(chap);
		render.then(function(chapter){
...
EPUBJS.Renderer.prototype.page = function(pg){
	//console.log(this.displayedPages); // 31
	if(pg >= 1 && pg <= this.displayedPages){
		this.chapterPos = pg;
		this.leftPos = this.spreadWidth * (pg-1); //-- pages start at 1
		//console.log(this.leftPos); // 2816
		this.setLeft(this.leftPos);
...
//-- Show the page containing an Element 定位到要显示的元素
EPUBJS.Renderer.prototype.pageByElement = function(el){
	var left, pg;
	if(!el) return;

	//console.log(el.getBoundingClientRect()); // DOMRect { left=2832, top=224, right=3346, 更多...}
	//console.log(this.spreadWidth); // 1408 如何计算的？
	left = this.leftPos + el.getBoundingClientRect().left; //-- Calculate left offset compaired to scrolled position
	pg = Math.floor(left / this.spreadWidth) + 1; //-- pages start at 1
	
	console.log("pageByElement line 4350",left, pg); //0
	this.page(pg);
...
EPUBJS.Renderer.prototype.getPageCfi = function(){
	// 有一个小BUG，有时会找不对当前显示元素
	var prevEl = this.visibileEl;
	//console.log("line 4425", prevEl.innerHTML);
	this.visibileEl = this.findFirstVisible(prevEl);
	//console.log("line 4427", this.visibileEl.innerHTML);
	try {
	...
	//console.log("line 4422", this.visibileEl, this.currentChapterCfi, this.pageIds);
	}catch(e){}
...
EPUBJS.Renderer.prototype.gotoCfiFragment = function(cfi){
	...
	//console.log(cfi); // Object

	if(_.isString(cfi)){
		cfi = this.epubcfi.parse(cfi);
	}
	
	element = render.epubcfi.getElement(cfi, render.doc);
	render.pageByElement(element);
};
...
EPUBJS.Renderer.prototype.findFirstVisible = function(startEl){
	...
	if(found) {
		//console.log("found el", found.innerHTML);
		return found;
	}else{
		//console.log("not found el", found.innerHTML);
		return startEl;
	}
