...
//				console.log("_pageX",_pageX,"_pageY",_pageY,"_clientX", _clientX);
		...
		position : function () {
			...
			$(span).show();
			$(span).css({height: "110px",width:spanWidth+'px',left:xx+'px',top:yy+'px'});
			setRangyPostion();
		}
	};
...
var if_serializedHighlights = decodeURIComponent(window.location.search.slice(window.location.search.indexOf("=") + 1));
var color_select = 0;
var highlighter;
var clip;
var sm;
var div_str = '<div id="selectTextPupupMenu" style="display:none">'
	...
	+'<li style="display: inline-block;"><a href="#" class="select-item add-underline" id="hilight-text-btn">标记</a></li>'
	+'<li style="display: inline-block;"><a href="#" class="select-item cancel-underline" id="unhilight-text-btn">清除标记</a></li>'
	+'<li><a href="#" id="note-text-btn" class="select-item modify-note share">笔记</a></li>'
	+'</ul>'
	+'<ul class="color-area">'
	...
	+'<textarea id="ta_note" style="margin:10px 20px 0px 20px" rows="4" cols="40"></textarea>'
	+'</div>';
...
function setLight(color_css){
	if("un"==color_css){
		highlighter.unhighlightSelection();
	}else{		
		highlighter.highlightSelection(color_css);
	}
	var ccc = highlighter.serialize();
	if_serializedHighlights = ccc;
	parent.rangy_hilighter_string = ccc;
	parent.doUpdateHilighter();
}

$(function() {
	$("body").wrapInner('<div id="contentDiv"></div>');
	$("body").append(div_str);
	...
	highlighter.addClassApplier(rangy.createClassApplier("colord"));
	 if (if_serializedHighlights && if_serializedHighlights !== null && if_serializedHighlights !== undefined && if_serializedHighlights !== '' && if_serializedHighlights !== "undefined") {
		 try{
             highlighter.deserialize(if_serializedHighlights);
		 }catch(e){
			 console.log("JJJJJLLLLL:   "+e);
		 }
     }
				...
				handler : function(e) {
					...
					if (id == "copy-text-btn") {
						...
						layer.open({
						    ...
						    btn: ['保存', '取消'],
						    yes: function(){
						    	var txt_note = $("#ta_note").val();
						    	var txt_select = rangy.getSelection().toString();
						    	parent.doUpdateNote(txt_note,txt_select);
						    	layer.msg('笔记保存完成!');
						    }
						});
