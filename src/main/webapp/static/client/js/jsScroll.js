...
function jsScroll(obj, w, className, id)
{
	...
	obj.scrollBar.id = id || ("scrollBar" + scrollDivList.length);
	if(!className) {
...
//		document.addEventListener('DOMMouseScroll',scrollMove,false); // firefox
//	}
...
function jScrollBarShow(div) {
	for(var i=0; i<scrollDivList.length; i++) {
		var o = scrollDivList[i];
		if (o.id == div) {
			scrollResetSize(o);
		}
	}
