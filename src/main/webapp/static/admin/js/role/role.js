var RoleManager = (function () {
        ...
        columns: [{
        ...
        },
            {
                ...
                formatter: function (value, row) {
                    ...
                    var s;
                    if (attribute == '1') {
                        s = "<span class='label label-dark'>后台角色</span>";
                    } else if (attribute == '2') {
                        s = "<span class='label label-warning' style='background-color: #D3AA9C'>前台角色</span>";
                    }
                    return s;
                }
            }, {
                ...
                formatter: function (value, row) {
                    ...
                    var s;
                    if (used == '0' && status == '0') {
                        s = "<span class='label label-success'>活动中</span>";
                    } else if (used == '1' || status == '1') {
                        s = "<span class='label label-warning'>未使用</span>";
                    }
                    return s;
                }
            }, {
    ...
    $('#remove-modal-btn-true').click(function () {
        ...
        var id = $("#id").val();
        $.ajax({
            url: appPath + '/admin/role/delete/' + id,
            dataType: 'json',
            type: 'post',
